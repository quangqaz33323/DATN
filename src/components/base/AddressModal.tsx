...
const AddressModal = ({ setShowAddressModal }: { setShowAddressModal: (show: boolean) => void }) => {
    ...
    return (
        <form onSubmit={e => toast.promise(handleSubmit(e), { loading: t('addAddress') })} className="fixed inset-0 z-50 bg-white/60 backdrop-blur h-screen flex items-center justify-center">
            <div className="flex flex-col gap-5 text-slate-700 w-full max-w-sm mx-6">
                <h2 className="text-3xl ">{t('addNew')}<span className="font-semibold">{t('address')}</span></h2>
                ...
                <button className="bg-slate-800 text-white text-sm font-medium py-2.5 rounded-md hover:bg-slate-900 active:scale-95 transition-all uppercase">{t('saveAddress')}</button>
            </div>
